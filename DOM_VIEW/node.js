...
function traverse(node){
	...
	for(let x=0;x<node_len;x++){
		...
		if(node.childNodes[x].nodeType == Node.ELEMENT_NODE || node.childNodes[x].nodeType == Node.DOCUMENT_TYPE_NODE){
			...
			if(node.childNodes[x].attributes){
				for(let i=0;i<node.childNodes[x].attributes["length"];i++){
					console.log(node.childNodes[x].attributes[i]);
					print_dom +=" "+node.childNodes[x].attributes[i].name+"="+node.childNodes[x].attributes[i].value+"</span></b>";
				}
			}
		}
		else{
